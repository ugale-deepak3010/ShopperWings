...
class ControllerExtensionPaymentKlarnaCheckout extends Controller {
	...
	public function validation() {
		...
		if ($validate) {
			...
			if ($order_info) {
				...
				if (round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100) != $request->order_amount) {
					$this->model_extension_payment_klarna_checkout->log('Klarna Checkout order_amount does not match session order total. Klarna Request: ' . $request->order_amount . '. shopperwings: ' . round($this->currency->format($total, $order_info['currency_code'], $order_info['currency_value'], false) * 100));
		...
		if ($validate) {
			...
			// Update shopperwings order with payment and shipping details
			...
			//If country isn't GB, try to update shopperwings order with correct region/zone
